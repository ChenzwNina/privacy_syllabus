...
from Functions.take_screenshots import take_pdf_screenshots, capture_screenshots

def get_content_and_url(url, class_folder, class_id):
    print(f"processing url get content ------------{url}")
    url_mappings = {}
    reason = ""
    print(url)
    ...
    if "google" in url.lower() or ".pdf" in url.lower():
        ...
        if output_path != "error":
            ...
            take_pdf_screenshots(output_path, class_folder, class_id)
            
    # If it is a website
    else: 
        syllabus_content, url_mappings, reason = parse_website_with_mappings(url)
        capture_screenshots(url, class_folder, class_id)
