...
def extract_with_images_text(select_class_number):
    ...
    for url in separate_url_list:
        ...
        temp_results, prompt_token_single, completion_token_single = send_images_text(class_folder, class_id, batch_size, syllabus_with_url)

        # Process API results.
        export_reading_from_image_text(temp_results, class_folder, class_id, url_mappings)

        final_prompt_token += prompt_token_single
        final_completion_token += completion_token_single
    ...
    print(f"[Image] The syllabus costs {final_prompt_token} prompt tokens")
    print(f"[Image] The syllabus costs {final_completion_token} completion tokens")
